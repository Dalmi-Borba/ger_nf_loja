...
const apiWoo = require('../helpers/apiWoo');
...
async function uploadXml(req, res) {
  ...
  const processamentos = arquivos.map(async (file) => {
    try {
      ...
      const resApi = await apiWoo(`${process.env.URL_API_WOO}${orderId}`);
      const pagtoType = resApi.pagtoType || '';
      const aut_card = resApi.aut_card || '';
      const dataPagtoFormat = resApi.dataPagtoFormat || '';
